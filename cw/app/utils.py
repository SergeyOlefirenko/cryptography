...
class DoubleRatchet:
    ...
    def encrypt_message(self, plaintext: bytes) -> Tuple[dict, bytes]:
        ...
        # Using a deterministic nonce based on Ns to avoid AES-GCM collisions
        ns_bytes = self.Ns.to_bytes(4, 'big')
        nonce = hkdf(NONCE_SIZE, mk, None, b"nonce" + ns_bytes)
    ...
    def decrypt_message(self, payload: dict) -> bytes:
        ...
        # Implements basic replay protection with skipped message key support
        ns = payload.get('ns', 0)
        if ns <= self.Nr:
            raise RuntimeError(f"Replay or out-of-order message detected: ns={ns}, Nr={self.Nr}")

        MAX_SKIP = 50
        if ns - self.Nr > MAX_SKIP:
            raise RuntimeError("Too many skipped messages")

        # Generating skipped keys
        while self.Nr < ns:
            mk_temp, self.recv_chain_key = self.kdf_ck(self.recv_chain_key)
            self.skipped_message_keys[self.Nr] = mk_temp
            self.Nr += 1
        # Remove used key from skipped_message_keys for forward secrecy
        if ns in self.skipped_message_keys:
            del self.skipped_message_keys[ns]
...
# UI integration
# These queues will be assigned by chat.py
#GUI_QUEUES = {"Alice": None, "Bob": None}
GUI_APP = None
GUI_QUEUES = {}  # name -> asyncio.Queue

async def gui_read_queue(name: str):
    # Read message from UI queue
    if GUI_QUEUES[name] is None:
        raise RuntimeError(f"No GUI queue for {name}")
    msg = await GUI_QUEUES[name].get()
    return msg

def gui_show_message(sender: str, msg: str):
    #Display message in GUI if available
    if GUI_APP:
        GUI_APP.append_message(sender, msg)
    else:
        show(msg)
